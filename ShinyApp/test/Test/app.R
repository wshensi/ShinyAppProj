...
server <- function(input, output, session) {
  ...
  # =============================================================================
  # TOP COLLABORATORS - 方法1：响应式数据 + 事件响应
  # =============================================================================
  
  # 1. 创建响应式数据函数，提供默认值
  collaborators_data <- reactive({
    # 提供默认值以防输入为NULL
    top_n <- if(is.null(input$collaborators_top_n)) 10 else input$collaborators_top_n
    include_performers <- if(is.null(input$collaborators_include_performers)) TRUE else input$collaborators_include_performers
    include_producers <- if(is.null(input$collaborators_include_producers)) TRUE else input$collaborators_include_producers
    ...
    tryCatch({
      ...
      # Build edge types based on user selections
      edge_types <- c("ComposerOf", "LyricistOf")  # Always include these
      if(include_performers) {
        edge_types <- c(edge_types, "PerformerOf")
      }
      if(include_producers) {
        edge_types <- c(edge_types, "ProducerOf", "RecordedBy")
      }
  ...
  output$plot_top_collaborators <- renderPlotly({
    ...
    if(nrow(data) == 0) {
      ...
      p <- ggplot(data, aes(x = reorder(name, times), y = times,
                            text = paste("Artist:", name, "<br>Genre:", coalesce(genre, "Unknown"), "<br>Collaborations:", times))) +
  ...
  # 3. 事件响应 - 监听按钮点击，使用响应式数据更新图表
  observeEvent(input$update_collaborators, {
    output$plot_top_collaborators <- renderPlotly({
      # 使用响应式数据
      ...
      if(nrow(data) == 0) {
        p <- ggplot() + 
          theme_void() + 
          labs(title = "No collaborator data available (Updated)") +
          theme(plot.title = element_text(hjust = 0.5, size = 14))
      } else {
        title_text <- paste0("Top Collaborators with Sailor Shift")
        if(!input$collaborators_include_performers && !input$collaborators_include_producers) {
          title_text <- paste0(title_text, " (Core Only)")
        } else if(input$collaborators_include_performers && input$collaborators_include_producers) {
          title_text <- paste0(title_text, " (All Types)")
        } else if(input$collaborators_include_performers) {
          title_text <- paste0(title_text, " (+ Performers)")
        } else {
          title_text <- paste0(title_text, " (+ Producers)")
        }
        ...
        p <- ggplot(data, aes(x = reorder(name, times), y = times,
          ...
          geom_col(fill = "darkorange", alpha = 0.8) +
          coord_flip() +
          labs(title = title_text, x = "Artist", y = "Number of Collaborations") +
          theme_minimal() +
          theme(
            text = element_text(size = 12),
            axis.text = element_text(size = 10),
...
output$plot_oceanus_timeline <- renderPlotly({
  ...
  if(length(oceanus_ids) == 0) {
    p <- ggplot() + theme_void() + labs(title = "No Oceanus Folk data available")
  } else {
