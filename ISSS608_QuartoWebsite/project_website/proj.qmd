...
```{r}
top_genres <- graph_nodes %>%
  count(group, sort = TRUE) %>%
  slice(1:3) %>%
  pull(group)

for (g in top_genres) {
  nodes_g <- graph_nodes %>% filter(group == g)
  edges_g <- linked_edges %>% filter(from %in% nodes_g$id | to %in% nodes_g$id)

  visNetwork(nodes_g, edges_g, height = "700px", width = "100%") %>%
    visEdges(arrows = "to", color = list(color = "gray")) %>%
    visOptions(highlightNearest = TRUE) %>%
    visLayout(randomSeed = 123) %>%
    visSave(file = paste0("graph_", g, ".html"))  # 保存为独立 HTML 文件
}
